...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:hcmc="http://hcmc.uvic.ca/ns/staticSearch" xmlns:svg="http://www.w3.org/2000/svg"
    ...
    <xd:doc>
        <xd:desc>Include the global config XSLT that is derived from the configuration file and the
            create_config.xsl. See <xd:a href="create_config_xsl.xsl">create_config_xsl.xsl</xd:a>
    ...
    <xd:doc>
        <xd:desc>This mode tag (which applies to the default) specifies that this is an identity
            transform (@on-no-match='shallow-copy'). This require XSLT 3.0.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Output as XHTML with HTML version 5.0; this is necessary for adding the propery
            DOCTYPE processing instruction.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc><xd:ref name="hasFilters">$hasFilters</xd:ref> is used to specify whether the site
            build process has discovered any filter metadata in the collection. If so, then we need
            to create appropriate form controls.</xd:desc>
    </xd:doc>
    <xsl:param name="hasFilters" as="xs:string" select="'false'"/>

    <xd:doc>
        <xd:desc><xd:ref name="docsJSON" type="variable">$docsJSON</xd:ref> is the previously
            created JSON file from the filters specified in the document metadata; we load this and
            then parse it as XML, since we create the filter selector from it.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc><xd:ref name="svgLogoFile">svgLogoFile</xd:ref> is the relative path from this XSLT
            to the file to be used as the staticSearch logo.</xd:desc>
    </xd:doc>
    ...
    <xsl:variable name="dateRegex"
        select="'^\d\d\d\d(-((((01)|(03)|(05)|(07)|(08)|(10)|(12))-((0[1-9])|([12][0-9])|(3[01])))|(((04)|(06)|(09)|(11))-((0[1-9])|([12][0-9])|(30)))|(02-((0[1-9])|([12][0-9]))))|(-((0[123456789])|(1[012]))))?$'"
        as="xs:string"/>

    <xd:doc>
        <xd:desc><xd:ref name="pageLang">pageLang</xd:ref> is the language of the search page (if
            specified) or "en" if not. This is used for sorting filter items.</xd:desc>
    </xd:doc>
    <xsl:variable name="pageLang" as="xs:string" select="
            if (/html/@*:lang) then
                /html/@*:lang[1]
            else
                'en'"/>
    ...
    <xd:doc>
        <xd:desc>Root template just for checking to see whether or not the document has the require
            an element with @id='staticSearch' (preferably a div).</xd:desc>
    </xd:doc>
    <xsl:template match="/">

        <!--Warning message if there is no staticSearch elements in the document. -->
        <xsl:if test="not(descendant::*[@id = 'staticSearch'])">
            <xsl:message>ERROR: Document does not contain an HTML element with @id='staticSearch'
                and thus this transformation will not do anything.</xsl:message>
        </xsl:if>
        ...
        <xsl:if test="
                descendant::*[@id = 'staticSearch'] and
                local-name(descendant::*[@id = 'staticSearch'][1]) ne 'div'">
            <xsl:message>WARNING: The element with @id='staticSearch' is not a div element. This may
                result in invalid HTML or unpredictable layout behaviour.</xsl:message>
        </xsl:if>
    ...
    <xd:doc>
        <xd:desc>This template replaces an existing style element with the id ssCss (old approach)
            or a link element with the same id (2020-01-21 onward) with a link element pointing to
            the CSS file.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>This template detects an HTML head element which does not contain an existing style
            element with the id ssCss, and injects one containing the code in the <xd:ref name="css"
                >$css</xd:ref> parameter. We place it first in the head element so that any
            subsequent style element provided by the user can override it.</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>This is the main template for matching the staticSearch element (which under normal
            circumstances should be a div, but which may be any element). This is where all the work
            happens for create the search box and the filter options. This is also where we load in
            the Javasript; we do this here rather than in the head of the document so not to
    ...
    <xsl:template match="*[@id = 'staticSearch'][not(preceding::*[@id = 'staticSearch'])]">
        ...
        <xsl:variable name="captionLang" as="xs:string">
            <xsl:variable name="declaredLang"
                select="string(ancestor-or-self::*[@xml:lang or @lang][1]/(@xml:lang, @lang)[1])"
                as="xs:string?"/>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:message use-when="$verbose">WARNING: No language declared for element with
                        @id='staticSearch' to determine captions. Using 'en' by
                        default.</xsl:message>
        ...
        <xsl:copy>
            ...
            <xsl:if
                ...
                <xsl:message>WARNING: Contents of element with @id='staticSearch' will be
            ...
            <noscript>
                <xsl:value-of select="hcmc:getCaption('ssScriptRequired', $captionLang)"/>
            </noscript>
            ...
            <form accept-charset="UTF-8" id="ssForm"
                ...
                data-resultsperpage="{$resultsPerPage}" onsubmit="return false;"
                data-versionstring="{$versionString}" data-ssfolder="{$outputFolder}"
                data-kwictruncatestring="{$kwicTruncateString}" data-resultslimit="{$resultsLimit}">
                ...
                <span class="ssQueryAndButton">
                    ...
                    <input type="text" id="ssQuery"
                        aria-label="{hcmc:getCaption('ssDoSearch', $captionLang)}"/>
                    <button id="ssDoSearch">
                        <xsl:sequence select="hcmc:getCaption('ssDoSearch', $captionLang)"/>
                    </button>
                </span>

                <xsl:if test="not(empty($filterJSONURIs)) or not(empty($ssContextMap))">
                    <xsl:variable name="descFilters"
                        select="$filterJSONURIs[matches(., 'ssDesc\d+.*\.json')]"/>
                    <xsl:variable name="featFilters"
                        select="$filterJSONURIs[matches(., 'ssFeat\d+.*\.json')]"/>
                    <xsl:variable name="dateFilters"
                        select="$filterJSONURIs[matches(., 'ssDate\d+.*\.json')]"/>
                    <xsl:variable name="boolFilters"
                        select="$filterJSONURIs[matches(., 'ssBool\d+.*\.json')]"/>
                    <xsl:variable name="numFilters"
                        select="$filterJSONURIs[matches(., 'ssNum\d+.*\.json')]"/>
                    ...
                    <xsl:if test="not(empty($ssContextMap))">
                        <xsl:variable name="caption"
                            select="hcmc:getCaption('ssSearchIn', $captionLang)"/>
                            ...
                            <fieldset class="ssFieldset" title="{$caption}">
                                ...
                                <ul class="ssSearchInCheckboxList">
                                    <xsl:for-each select="map:keys($ssContextMap)">
                                        ...
                                        <xsl:sort select="."/>
                                        <xsl:variable name="currLabel" select="." as="xs:string"/>
                                        <xsl:variable name="id" select="$ssContextMap(.)"
                                            as="xs:string"/>
                                        <li>
                                            <input type="checkbox" title="{$currLabel}"
                                                value="{$currLabel}" id="{$id}"
                        ...
                        <div class="ssDescFilters">
                            ...
                            <xsl:variable name="fieldsets" as="element(fieldset)*">
                                <xsl:for-each select="$descFilters">
                                    ...
                                    <fieldset class="ssFieldset" title="{$filterName}"
                                        id="{$filterId}">
                                        <legend>
                                        ...
                                        <ul class="ssDescCheckboxList">
                                            ...
                                            <xsl:variable name="notNumeric" select="
                                                    some $n in (for $s in $jsonDoc//j:map[@key]/j:string[@key = 'sortKey']
                                                    return
                                            ...
                                            <xsl:variable name="sortedMaps" as="element(j:map)+">
                                                <xsl:choose>
                                                  ...
                                                  <xsl:sort
                                                  select="replace(j:string[@key = 'sortKey'], '^((the)|(a)|(an)|(l[ea]s?)|(de[nrs]?)|([ie]l)|(un[oe]?))\s+', '', 'i')"
                                                  lang="{$pageLang}"/>
                                                  <xsl:sequence select="."/>
                                                  </xsl:for-each>
                                                  </xsl:when>
                                                  <xsl:otherwise>
                                                  <xsl:for-each select="$jsonDoc//j:map[@key]">
                                                  <xsl:sort select="j:string[@key = 'sortKey']"
                                                  data-type="number"/>
                                                  <xsl:sequence select="."/>
                                                  </xsl:for-each>
                                                  </xsl:otherwise>
                                            ...
                                            * an @id to associate the label for A11Y-->
                                                ...
                                                <li>
                                                  ...
                                                  <input type="checkbox" title="{$filterName}"
                                                  value="{$thisOptName}" id="{$thisOptId}"
                                ...
                                <xsl:for-each select="$featFilters">
                                    ...
                                    <fieldset class="ssFieldset" title="{$filterName}"
                                        ...
                                        <input type="text" title="{$filterName}"
                                            placeholder="{hcmc:getCaption('ssStartTyping', $captionLang)}"
                        ...
                        <div class="ssDateFilters">
                            ...
                            <xsl:variable name="fieldsets" as="element(fieldset)*">
                                <xsl:for-each select="$dateFilters">
                                    ...
                                    <xsl:variable name="minDate" as="xs:date" select="
                                            min((for $d in $jsonDoc//j:string[1][matches(., $dateRegex)]
                                            return
                                                hcmc:normalizeDateString($d, true())))"/>

                                    <!--And the maximum date-->
                                    <xsl:variable name="maxDate" as="xs:date" select="
                                            max((for $d in $jsonDoc//j:string[1][matches(., $dateRegex)]
                                            return
                                                hcmc:normalizeDateString($d, false())))"/>
                                    ...
                                    <fieldset class="ssFieldset" title="{$filterName}"
                                        ...
                                        <span>
                                            ...
                                            <input type="text" maxlength="10" pattern="{$dateRegex}"
                                                title="{$filterName}" id="{$filterId}_from"
                                                class="staticSearch.date staticSearch_date"
                                                placeholder="{format-date($minDate, '[Y0001]-[M01]-[D01]')}"
                                        ...
                                        <span>
                                            ...
                                            <input type="text" maxlength="10" pattern="{$dateRegex}"
                                                ...
                                                class="staticSearch.date staticSearch_date"
                                                placeholder="{format-date($maxDate, '[Y0001]-[M01]-[D01]')}"
                                                onchange="this.reportValidity()"/>
                                        </span>
                                    </fieldset>
                                </xsl:for-each>
                            ...
                            <xsl:variable name="fieldsets" as="element(fieldset)*">
                                ...
                                <xsl:for-each select="$numFilters">
                                    ...
                                    <xsl:variable name="vals"
                                        select="$jsonDoc//j:string[not(@key)][. castable as xs:decimal]/xs:decimal(.)"/>
                                ...
                                <xsl:variable name="spans" as="element(span)*">
                                    <xsl:for-each select="$boolFilters">
                                        ...
                                        <xsl:variable name="filterId"
                                            select="$jsonDoc//j:string[@key = 'filterId']"/>
                                        <span>
                                            <label for="{$filterId}"><xsl:value-of
                                                  select="$filterName"/>: </label>
                                            <select id="{$filterId}" title="{$filterName}"
                                                class="staticSearch.bool staticSearch_bool">
                                                <option value="">?</option>
                                                <!-- Check mark = true -->
                    ...
                    <span class="clearButton">
                        <button id="ssClear2" onclick="Sch.clearSearchForm(); return false;">
                            <xsl:sequence select="hcmc:getCaption('ssClear', $captionLang)"/>
                        </button>
                    </span>
                </xsl:if>
            ...
            <div id="ssPoweredBy">
                ...
                <a href="https://github.com/projectEndings/staticSearch"
                    aria-label="{hcmc:getCaption('ssPoweredBy', $captionLang)} staticSearch">
    ...
    <xd:doc>
        <xd:desc>Repeated running of this process over the same document can result in excessive
            quantities of whitespace. Rather than try to figure out where to preserve whitespace and
            where not, we just constrain it with this template.</xd:desc>
    </xd:doc>
    <xsl:template
    ...
    <xd:doc>
        <xd:desc><xd:ref name="hcmc:normalizeDateString" type="function"
                >hcmc:normalizeDateString</xd:ref> converts truncated dates (yyyy, or yyyy-mm) to
            fully-specified dates. It ignores leap years. </xd:desc>
        <xd:param name="dateString">dateString is the incoming string representation of a
            date.</xd:param>
        <xd:param name="earliest">earliest is a boolean parameter specifying whether it should be
            the earliest possible date or the latest.</xd:param>
    ...
    <xsl:function name="hcmc:normalizeDateString" as="xs:date?">
        ...
        <xsl:choose>
            <xsl:when test="matches($dateString, '^\d\d\d\d$')">
                <xsl:choose>
                    <xsl:when test="$earliest">
                        <xsl:sequence select="xs:date(concat($dateString, '-01-01'))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:sequence select="xs:date(concat($dateString, '-12-31'))"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:when test="matches($dateString, '^\d\d\d\d-\d\d$')">
                <xsl:choose>
                    <xsl:when test="$earliest">
                        <xsl:sequence select="xs:date(concat($dateString, '-01'))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:choose>
                            <xsl:when
                                test="matches($dateString, '^\d\d\d\d-((01)|(03)|(05)|(07)|(08)|(10)|(12))$')">
                                <xsl:sequence select="xs:date(concat($dateString, '-31'))"/>
                            </xsl:when>
                            <xsl:when
                                test="matches($dateString, '^\d\d\d\d-((04)|(06)|(09)|(11))$')">
    ...
    <xd:doc>
        <xd:desc>We need to prevent the proliferation of namespaces which make things
            invalid.</xd:desc>
    </xd:doc>
